...
def get_data_from_file(file_path):
    path = './data/collected_comments/' + file_path
    with open(path, 'r', encoding='utf-8')as f:
        return f.read()


def data_from_dict(comments_dict):
    comments = set()
    posts = set()
    for post_id in comments_dict:
        for i in range(1, len(comments_dict[post_id])):
            ...
            for comment in comment_array:
                comment_to_add = '%s\t%s' % (comment['from_id'], clean_line(comment['text']))
...
def main():
    comments_total = set()
    posts_total = set()
    for file in os.listdir('./data/collected_comments'):
        if file.endswith('.json'):
            comments_dict = json.loads(get_data_from_file(file))
            posts_json, comments_json = data_from_dict(comments_dict)
