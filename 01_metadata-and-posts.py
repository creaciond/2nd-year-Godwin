...
def api_comments(set_comments, params, offset):
    ...
    if response:
        for i in range(1, len(response.json()['response'])):
            ...
            '''
                group id = params['owner_id']
                from whom is the comment — comment['from_id']
                text of the comment — comment['text']
            '''
            line = params['owner_id'].strip('-') + '\t' + str(comment['from_id']) + '\t' + clean_line(comment['text'])
...
def get_comments(group_info):
    ...
    for post_id in post_ids:
        ...
        if response.status_code == 200:
            print('post %d, comments length: %s, percentage done: %f per cent' % (
                count, response.json()['response'][0], count / len(post_ids) * 100))
